...
public class jdbc {
    ...
    /*public void tableOut() throws SQLException{
        String selectTableSQL = "SELECT username,message,password,login,last_entry,surname from user_list";
        dbConnection = this.establishConnection();
        statement = dbConnection.createStatement();
        ResultSet rs = statement.executeQuery(selectTableSQL);

        System.out.println("Username        message        password     login   last_entry    surname");
        while (rs.next()) {
            String username = rs.getString("username");
            String message = rs.getString("message");
